#########################################################################################
##
##            PathSim example: parameter estimation for a tritium reactor model
##                          (originally built with PathView)
##
##  Model:   tritium inventory in reactor vessel walls and top shield
##  Fit:     wall permeation coefficient k_wall and top permeation coefficient k_top
##           both estimated in log10 space via exp10 transform
##
#########################################################################################
...
# Sources
p383       = Constant(value=4.95e8 / 2)
a325       = Constant(value=2.13e8 / 2)
tbr        = Constant(value=5.4e-4)
k_wall     = Constant(value=1.2)
k_top      = Constant(value=4.0)
stepsource = StepSource()
block_6    = StepSource(amplitude=-1, tau=12 * 3600)
block_7    = StepSource(tau=24 * 3600)
block_8    = StepSource(tau=36 * 3600, amplitude=-1)
a_top      = Constant(value=1e1)
a_wall     = Constant(value=1e2)
...
block_14    = Adder(operations="+--")
s           = Multiplier()
block_16    = Adder(operations="++")
block_17    = Adder(operations="++")
irradiations = Adder(operations="++")
multiplier  = Multiplier()
block_20    = Multiplier()
block_21    = Multiplier()
amplifier   = Amplifier(gain=1e-6)
block_23    = Amplifier(gain=1e-8)
block_24    = Amplifier(gain=1e-9)
...
if __name__ == '__main__':
    ...
    # Synthetic measurement data (tritium inventory, first output of q_int)
    sec2day = 1 / (24 * 3600)
    t_meas = np.r_[0.5, 1, 2, 3, 4, 5, 6] * 24 * 3600
    y_meas = np.r_[4, 11, 17, 24, 24, 24, 24] / 2 + (np.random.random(7) - 0.5)

    meas = TimeSeriesData(time=t_meas, data=y_meas, name="q_wall")

    # Create the estimator and register block parameters in log10 space
    est = ParameterEstimator(simulator=sim, adaptive=True)

    est.add_block_parameter(
        k_wall, 'value', param_id='k_wall',
        value=0.2, bounds=(-1, 1), transform=exp10,
    )
    est.add_block_parameter(
        k_top, 'value', param_id='k_top',
        value=0.3, bounds=(-1, 1), transform=exp10,
    )
    ...
    # Custom plot with time axis in days
    t_pred, y_pred = est.simulate(fit.x)

    plt.figure(figsize=(8, 5))
    plt.plot(t_meas * sec2day, y_meas,  'o', ms=5, alpha=0.6, label='Measured')
    plt.plot(t_pred * sec2day, y_pred,  '-', lw=2,             label='Fit')
    plt.xlabel('Time [d]')
    plt.ylabel('Tritium inventory [a.u.]')
    plt.title('Tritium reactor — parameter fit')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.show()

    # Also show the built-in plot_fit view
    fig, axes = est.plot_fit(
        fit.x,
        title="Tritium fit",
        xlabel="Time [s]",
        ylabel="Tritium inventory [a.u.]",
    )
    plt.show()
